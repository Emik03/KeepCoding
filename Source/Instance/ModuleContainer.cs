    ...
    public sealed class ModuleContainer : IEquatable<ModuleContainer>
    {
        ...
        public int GetRuleSeedId(int editorValue)
        {
            ...
            if (ruleSeedDictionary.ContainsKey("AddSupportedModule"))
                ruleSeedDictionary["AddSupportedModule"] = Id;
        ...
        public string[] GetIgnoredModules()
        {
            ...
            {
                logger.Log($"Boss Module Manager does not have a list on record for “{Name}”.");
                return new string[0];
            }

            string[] list = ((Func<string, string[]>)dictionary[Key])(Name);

            logger.Log($"Boss Module Manager returned list for “{this}”: {(list is null ? Helper.Null : list.Combine())}");
